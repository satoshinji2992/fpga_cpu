//==================================================
// Stage 7 regression: CSR read (RDCYCLE/RDINSTRET) + ECALL.
//   RDCYCLE   x3  -> x3 = cycle counter
//   RDINSTRET x4  -> x4 = instret counter
//   SW them to Mem[0]/Mem[1], then ECALL (halt).
//==================================================
`timescale 1ns/1ps
module tb_csr;
    ...
    initial begin
        ...
        instr_mem[2] = 32'hC00021F3; // RDCYCLE   x3  (CSRRS x3, 0xC00, x0)  -> x3 = cycle
        instr_mem[3] = 32'h00302023; // SW        x3, 0(x0)   -> Mem[0] = cycle
        instr_mem[4] = 32'hC0202273; // RDINSTRET x4  (CSRRS x4, 0xC02, x0)  -> x4 = instret
        instr_mem[5] = 32'h00402223; // SW        x4, 4(x0)   -> Mem[1] = instret
        instr_mem[6] = 32'h00000073; // ECALL (halt)
    end
    ...
    initial begin
        ...
        $display("");
        $display("=======================================");
        $display("CSR HALT=%0d (ECALL)", halt);
        $display("Mem[0] = %0d (RDCYCLE,   must be >0)",        data_mem[0]);
        $display("Mem[1] = %0d (RDINSTRET, must be >0, <=cycle)", data_mem[1]);
        if (halt && (data_mem[0] > 0) && (data_mem[1] > 0) && (data_mem[0] >= data_mem[1]))
            $display("CSR PASS");
        else
            $display("CSR FAIL");
