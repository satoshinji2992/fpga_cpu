//==================================================
// Stage 4 regression: RV32M multiply/divide.
//   MUL  x3 = 7*6 = 42
//   DIV  x4 = 7/6 = 1
//   REM  x5 = 7%6 = 1
//==================================================
...
module tb_muldiv;
    ...
    initial begin
        ...
        instr_mem[3] = 32'h0220C233; // DIV  x4, x1, x2  -> 1
        instr_mem[4] = 32'h0220E2B3; // REM  x5, x1, x2  -> 1
        instr_mem[5] = 32'h00302023; // SW   x3, 0(x0)   -> Mem[0] = 42
        instr_mem[6] = 32'h00402223; // SW   x4, 4(x0)   -> Mem[1] = 1
        instr_mem[7] = 32'h00502423; // SW   x5, 8(x0)   -> Mem[2] = 1
        instr_mem[8] = 32'h0000006F; // JAL  x0, 0 (halt)
    end
    ...
    initial begin
        ...
        $display("");
        $display("=======================================");
        $display("MULDIV HALT=%0d", halt);
        $display("Mem[0] = %0d (expected 42, MUL 7*6)",  data_mem[0]);
        $display("Mem[1] = %0d (expected 1,  DIV 7/6)",  data_mem[1]);
        $display("Mem[2] = %0d (expected 1,  REM 7%%6)",  data_mem[2]);
        if (halt && (data_mem[0] == 32'd42) && (data_mem[1] == 32'd1) && (data_mem[2] == 32'd1))
            $display("MULDIV PASS");
        else
            $display("MULDIV FAIL");
        $display("PERF cycle=%0d instret=%0d mdu_inst=%0d", perf_cycle, perf_instret, perf_mdu);
        $display("=======================================");
        $finish;
    end
