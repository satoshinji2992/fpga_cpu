...
module tb_cpu_core;
    ...
    always @(posedge clk) begin
        if (data_we) begin
            ...
            $display("[%0t] WRITE addr=%08h data=%08h be=%b",
    ...
    //----------------------------------------------
    // 测试程序加载
    // 覆盖题目B基础要求中的算术、访存、分支跳转。
    // 结束后:
    //   Mem[0] = 0x34801200, 验证非0偏移SB
    //   Mem[1] = 0x0000FFFE, 验证SH
    //   Mem[2] = 2,          验证BEQ/BGE跳转结果
    //----------------------------------------------
    initial begin
        // 等待复位完成后再加载程序
        #1;
    ...
    initial begin
        ...
        if (halt) begin
            ...
            $display("x1  = %08h (expected ffffffff)", u_cpu.regfile[1]);
            $display("x3  = %08h (expected 00000012)", u_cpu.regfile[3]);
            $display("x6  = %08h (expected ffffff80)", u_cpu.regfile[6]);
            $display("x7  = %08h (expected 00000080)", u_cpu.regfile[7]);
            $display("x11 = %08h (expected fffffffe)", u_cpu.regfile[11]);
            $display("x12 = %08h (expected 0000fffe)", u_cpu.regfile[12]);
            if ((data_mem[0] == 32'h34801200) &&
                (data_mem[1] == 32'h0000FFFE) &&
                (data_mem[2] == 32'd2) &&
                (u_cpu.regfile[1]  == 32'hFFFFFFFF) &&
                (u_cpu.regfile[3]  == 32'h00000012) &&
                (u_cpu.regfile[6]  == 32'hFFFFFF80) &&
                (u_cpu.regfile[7]  == 32'h00000080) &&
                (u_cpu.regfile[11] == 32'hFFFFFFFE) &&
                (u_cpu.regfile[12] == 32'h0000FFFE))
                $display("PASS");
            else
                $display("FAIL");
            $display("=======================================");
        end else begin
            $display("ERROR: CPU timeout - did not halt");
        end
        $finish;
    end
