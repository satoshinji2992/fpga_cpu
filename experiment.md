...
| 串口 | 115200 baud，8N1 |
| 完整仿真回归 | `17/17 PASS` |
| 当前板端结论 | R16已在50 MHz真板通过开机自检、CNN、计算器、SDRAM、Pong、Paint及全部交互功能验收 |
...
该段为R14历史实现记录：XST确认`2048x32`指令ROM和四个`1024x8`数据RAM均实现为Block RAM。后续R16已扩展为`4096x32`指令ROM，并完成50 MHz真板全部功能验证。
...
2. R15已解决寄存器文件被XST错误映射为同步BRAM的问题；R16扩容后继续保持Distributed RAM映射，并已完成ISE和50 MHz真板验收。
...
1. 保存R16最终串口、LED、ISE和开发板演示证据。
2. 若继续增加功能，优先控制Occupied Slices 95%和0.651 ns时序裕量。
3. 如需更精确功耗，导入代表性VCD/SAIF或进行板级电流测量。
4. 后续版本继续保持寄存器文件Distributed RAM映射和同步BRAM访问时序约束。
